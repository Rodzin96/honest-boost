...
const API_BASE_URL = (process.env.HONEST_BOOST_API_BASE_URL || 'http://localhost:3000').replace(/\/$/, '');
...
function postJson(url, data) {
  return new Promise((resolve, reject) => {
    ...
    req.on('timeout', () => req.destroy(new Error('Tempo esgotado ao conectar ao servidor.')));
    req.on('error', reject);
    req.end(body);
  });
}
...
// App authentication: validate keys generated by the website backend.
